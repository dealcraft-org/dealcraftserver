...
//const firebase = require("firebase-admin/lib/auth");
const constants = require("../constants");
const {runtimeOptsUploadImg} = require("../var");
const getVehDetails = require("./getvehiclefulldata.js");
...
const ownerInvite = {
  ...
  uuidCompanion: "",
  path: "",
  set current(invitePropertyList) {
    this.docRefID = invitePropertyList.docRefID;
    this.role = invitePropertyList.role;
    this.uuidCompanion = invitePropertyList.uuidCompanion;
  },
  set setDocRefPath(docRefPath) {
    this.path = docRefPath;
  },
};
...
exports.uploadImage = functions.runWith(runtimeOptsUploadImg).https.onRequest((request, response) => {
    ...
    if (!continueExecution) {
      ...
      busboy.on("file", function(fieldname, file, filename, encoding, mimetype) {
        ...
        function cropPlateImgUpload() {
          ...
          return initPlateImgsPromise = new Promise(function(resolve) {
            ...
            functions.logger.debug("pst upload plate");
        ...
        function uploadPlateAndInitCarImage() {
          ...
          return new Promise(function(resolve) {
            ...
            functions.logger.debug("pst upload");
            mainProfilePicUrl = getPublicImgUrl(ref, carObject.uuid);
            mainProfilePic = ref.metadata.name;
            Promise.resolve("sss");
          }).then(() => {
            // crop upload
        ...
        async function writeInitFireStoreData(ownerInvite = undefined) {
...
// setting invite ownership information
            ...
            ownerInvite.setDocRefPath = pathToMaster;
            // clean for serialization //
            delete ownerInvite.current;
            delete ownerInvite.setDocRefPath;
            // finish invites
          ...
          case methods.DELIMGMAIN: {
            getDocRefID().then(docRefID => {
              if (docRefID.length == 0) {
                functions.logger.error(`no ref doc found for docRefID${formparam["docRefID"]}`);
          ...
          case methods.CHGIMGMAIN:
            ...
            getDocRefID().then(docRefID => {
              ...
              if (docRefID.length == 0) {
                ...
                let fileChangedName = null;
                let pathtoImg = getPathToImage(docRefID);


                let v = new Promise((resolve, reject) => updateImages(imageDataClean, pathtoImg, origFileName, formparam, resolve)).then(fileWritten => {
              ...
              return new Promise((resolve, reject) => {
                updateImages(imageDataClean, getPathToImage(docRefID), origFileName, formparam, resolve)
                  .then(f => {
                    ...
                    }).then(() => functions.logger.info("passpord uploaded" + f.name));
                    return resolve(f);
                  })
            ...
            async function postImage(galId, imgType) {
              ...
              function getImageDataClean() {
                const tempRes = imgFilesBundle.filter((value, index) => {
                  if (galId == value.index) {
                    console.log(`match:${galId} ${value.index}`);
                ...
                return new Promise(async (resolve, reject) => {
                  ...
                  await  new Promise(async (resolve, reject) => {updateImages(imgBase64,
                    ...
                    .then(async f => {
                      ...
                      }).then(() => functions.logger.info("updated" + f.name + "id:" + galId));
                      // GET ARRAY //
                      // UPLOAD ARRAY //
                      // UPDATE ARRAY //
                      // RETURN CODE //
